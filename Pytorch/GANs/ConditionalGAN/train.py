...
from utils import GradientPanelty
...
gen = model.Generator(z_dim , img_channels , features_g , num_classes , img_size , gen_embedding).to(device)

critic = model.Critic(img_channels , features_d , num_classes , img_size).to(device)
...
for epoch in range(epochs):
    ...
    for batch_idx , (real , label) in enumerate(p_bar):
        ...
        for i in range(critic_iterations):
            ...
            fake_out =critic(fake,label).reshape(-1)
            real_out = critic(real,label).reshape(-1)

            GP = GradientPanelty(real , fake , critic ,label , device)

            critic_loss = (
                -(torch.mean(real_out) - torch.mean(fake_out)) + lambda_gp*GP
            )
        ...
        gen.zero_grad()
        output = critic(gen(noise,label),label).reshape(-1)
